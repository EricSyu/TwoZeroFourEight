    ...
    private void drawCells(Canvas canvas) {
        ...
        for (int xx = 0; xx < game.numSquaresX; xx++) {
            for (int yy = 0; yy < game.numSquaresY; yy++) {
                ...
                Component currentComponent = game.grid.getCellContent(xx, yy);
                if (currentComponent != null) {
                    //Get and represent the value of the tile
                    int value = currentComponent.getValue();
                    int index = log2(value);

                    //Check for any active animations
                    ArrayList<AnimationRect> aArray = game.aGrid.getAnimationCell(xx, yy);
                    ...
                    for (int i = aArray.size() - 1; i >= 0; i--) {
                        ...
                        if (aCell.getAnimationType() == MainGame.SPAWN_ANIMATION) { // Spawning animation
                            ...
                            int currentX = currentComponent.getX();
                            int currentY = currentComponent.getY();
